...
const CalculateurPatesPapiers = () => {
  ...
  const [ui, setUi] = useState({
    ...
    ongletActif: 'general',
    exportPDFEnCours: false,
    nomEntreprise: '',
    showNomEntrepriseInput: false
  });

  // Référence pour la section à exporter en PDF
  ...
  const exporterPDF = async () => {
    setUi(prev => ({ ...prev, exportPDFEnCours: true }));
    ...
    try {
      ...
      // Générer le PDF
      await PDFExport.genererPDFPatesPapiers(
        resultats, 
        parametresExport, 
        typeSystemeActuel,
        ui.nomEntreprise
      ...
      // Réinitialiser l'UI après l'export
      setUi(prev => ({ 
        ...prev, 
        exportPDFEnCours: false,
        showNomEntrepriseInput: false
      ...
      setUi(prev => ({ ...prev, exportPDFEnCours: false }));
      alert('Une erreur est survenue lors de la génération du PDF. Veuillez réessayer.');
    }
  };

  // Afficher/masquer l'input du nom d'entreprise
  const toggleNomEntrepriseInput = () => {
    setUi(prev => ({ 
      ...prev, 
      showNomEntrepriseInput: !prev.showNomEntrepriseInput 
    }));
  };

  // Gestionnaire pour l'entrée du nom d'entreprise
  const handleNomEntrepriseChange = (e) => {
    setUi(prev => ({ ...prev, nomEntreprise: e.target.value }));
  };
  ...
  const { afficherDetails, ongletActif, exportPDFEnCours, showNomEntrepriseInput } = ui;
      ...
      <div className="mb-8 bg-gradient-to-r from-green-50 to-blue-50 p-6 rounded-lg border border-green-200">
        <div className="flex justify-between items-start">
          <div>
            ...
            {/* Bouton d'export PDF */}
            <div className="mt-4">
              {showNomEntrepriseInput ? (
                <div className="flex items-center space-x-2 mb-2">
                  <input
                    type="text"
                    value={ui.nomEntreprise}
                  ...
                  <button
                    onClick={toggleNomEntrepriseInput}
